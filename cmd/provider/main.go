...
func main() {
	...
	kingpin.FatalIfError(apiscluster.AddToScheme(scheme), "Cannot add cluster-scoped AzAPI APIs to scheme")
	kingpin.FatalIfError(apisnamespaced.AddToScheme(scheme), "Cannot add namespaced AzAPI APIs to scheme")
	...
	if *enableChangeLogs {
		...
		clo := xpcontroller.ChangeLogOptions{
			ChangeLogger: managed.NewGRPCChangeLogger(
				...
				managed.WithProviderVersion(fmt.Sprintf("provider-upjet-azapi:%s", version.Version))),
		}
